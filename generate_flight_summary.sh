...
# Generate flight summary CSV using DuckDB CLI with proper flight segmentation
echo "Generating flight summary CSV with multiple flights per day..."
...
WITH unnested AS (
    ...
    FROM read_json('docs/data/flight-track-*.json', filename=true, format='unstructured', ignore_errors=true)
),
altitudes AS (
    SELECT 
        flight_date,
        ...
        COALESCE(TRY_CAST(point[4] AS INTEGER), 0) as altitude,
        CASE WHEN COALESCE(TRY_CAST(point[4] AS INTEGER), 0) < 2000 THEN 1 ELSE 0 END as is_ground
    FROM unnested
),
transitions AS (
    SELECT 
...
flight_details AS (
    SELECT DISTINCT
        ...
        FIRST_VALUE(s.altitude) OVER (PARTITION BY e.flight_date, e.flight_num ORDER BY s.idx) as takeoff_altitude,
        -- Get landing details
        ...
        LAST_VALUE(s.altitude) OVER (PARTITION BY e.flight_date, e.flight_num ORDER BY s.idx
...
" > docs/data/flight-summary.csv

echo "Flight summary CSV generated successfully!"
echo "Rows generated: $(wc -l < docs/data/flight-summary.csv)"